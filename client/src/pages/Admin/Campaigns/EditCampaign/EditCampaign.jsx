...
const url = process.env.REACT_APP_CAMPAING_SERVICE_URL || "http://localhost:80";
...
function EditCampaign() {
  ...
  const ItemhandleSubmit = (e) => {
    ...
    if (startDate > endDate) {
      setErrorMessage(
        "A Data Inicial Não Pode Ser Inferior a Data de Expiração!"
      );
      return;
    }
        ...
        <div className="inputwrap">
          <form onSubmit={ItemhandleSubmit}>
            ...
            <Link to={`/discounts/campaigns`}>
              <button className="button cancel m-2">Cancelar</button>
            </Link>
          </form>
        </div>
      </div>
